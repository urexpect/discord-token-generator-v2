...
class Discord:
    def createSession(self, proxy) -> bool:
        ...
        if proxy is not None:
            self.session.proxies.update({'http': 'http://' + proxy, 'https': 'https://' + proxy})  # ip:port OR user:pass@ip:port
            try:
    ...
    def tryRegister(self, date_of_birth):
        t = self.session.post('https://discord.com/api/v9/auth/register', headers={
    ...
    def requestSms(self, captcha_key, number):
        ...
        response = self.session.post('https://discord.com/api/v9/users/@me/phone',
                                     headers={'referer': 'https://discord.com/channels/@me', 'authorization': self.token}, json={
                'captcha_key': captcha_key,
    ...
    def submitSms(self, code, number):
        token = self.session.post('https://discord.com/api/v9/phone-verifications/verify',
                                  headers={'referer': 'https://discord.com/channels/@me', 'authorization': self.token}, json={
                'code': code,
                'phone': '+' + number
            }).json()
        ...
        if response.status_code != 204:
            Exception("Something error occured on SMS verification")

    def process_img(self, img):
        image = open(img, 'rb')
        image_64_encode = "data:image/png;base64," + str(encodebytes(image.read()), 'UTF-8')
        image.close()
        return image_64_encode

    def uploadAvatar(self, img):

        image_data = self.process_img(img)
        respone = self.session.patch(' https://discord.com/api/v9/users/@me', headers={
            'authorization': self.token,
            'referer': 'https://discord.com/channels/@me',
            "sec-ch-ua": "\" Not;A Brand\";v=\"99\", \"Firefox\";v=\"91\", \"Chromium\";v=\"91\"",
            "sec-ch-ua-mobile": "?0"
    ...
    def __init__(self, proxy, verbose) -> None:
        ...
        if (not self.createSession(proxy)):
            Exception('Somethin wrong proxy!')
        self.getFingerPrint()
